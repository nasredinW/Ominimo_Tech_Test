...
from pyspark.sql.functions import concat_ws, lit, when, coalesce
...
class DataQualityValidator:
    ...
    def validate(df: DataFrame, rules: List[Dict[str, Any]]) -> Tuple[DataFrame, DataFrame]:
        ...
        for rule in rules:
            ...
            for validator_config in validators:
                try:
                    ...
                    field_error_labels.append(when(~condition, lit(error_msg)))
        ...
        invalid_df = df.filter(~final_condition) \
            .withColumn(
                "validation_errors",
                coalesce(concat_ws("; ", *all_error_labels), lit("Unknown validation error"))
            )
